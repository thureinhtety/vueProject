<template>
  <v-card class="pa-10 mt-10" max-width="500px">
    <v-form ref="form" v-model="valid">
      <h3>Update User</h3>
      <img :src="user.showImg ? user.showImg : VUE_IMG_URL+user.profile" width="150" height="150">
      ...
      <v-select :items="items" v-model="selected" label="Type" outlined :rules="typeRules"></v-select>
      ...
      >
        ...
        <v-date-picker v-model="user.dob" no-title scrollable>
          <v-btn text color="primary" @click="$refs.menu1.save(dob)">OK</v-btn>
        </v-date-picker>
      </v-menu>
      ...
      <v-btn :disabled="!valid" @click="showUpdateConfirmUser" class="mr-5">confirm</v-btn>
      <v-btn @click="clear">clear</v-btn>
    </v-form>
  </v-card>
</template>

<script src="../../services/pages/user/user-update.js">
</script>